...
# ── Transport suite ──
# Local haiku adaptation: original tests Portal's aika.sh; skip if not present.
# Use run-haiku.sh for Haiku transport tests.
...
run_transport_suite() {
  if [ ! -f "$SCRIPT_DIR/transport-evals.sh" ] || [ ! -f "$PLUGIN_DIR/scripts/lib/aika.sh" ]; then
    echo ""
    echo "Transport (scripts/lib/aika.sh, stubbed portal-cli) — skipped (no aika.sh, use run-haiku.sh for Haiku)"
    echo "────────────────────────────────────────────────────────────────"
    printf "  \033[33mSKIP\033[0m  %-32s Haiku mode: run bash shunt-evals/run-haiku.sh instead\n" "transport-evals"
    return
  fi
  echo ""
